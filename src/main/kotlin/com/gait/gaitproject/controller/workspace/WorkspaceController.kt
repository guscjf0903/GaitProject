...
class WorkspaceController(
    ...
    @PostMapping
    fun create(@Valid @RequestBody request: WorkspaceCreateRequest): ResponseEntity<ApiResponse<WorkspaceResponse>> =
        ResponseEntity.ok(ApiResponse.ok(workspaceService.create(request)))

    @Operation(summary = "워크스페이스 조회", description = "workspaceId로 상세 조회합니다.")
    @GetMapping("/{workspaceId}")
    fun get(@PathVariable workspaceId: UUID): ResponseEntity<ApiResponse<WorkspaceResponse>> =
        ResponseEntity.ok(ApiResponse.ok(workspaceService.getResponse(workspaceId)))
}
