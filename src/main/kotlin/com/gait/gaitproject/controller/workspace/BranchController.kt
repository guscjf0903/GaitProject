...
class BranchController(
    ...
    fun create(
        ...
        ResponseEntity.ok(ApiResponse.ok(branchService.create(request.copy(workspaceId = workspaceId))))

    @Operation(summary = "워크스페이스의 브랜치 목록", description = "workspaceId로 브랜치 목록을 조회합니다.")
    @GetMapping
    fun listByWorkspace(@PathVariable workspaceId: UUID): ResponseEntity<ApiResponse<List<BranchResponse>>> =
        ResponseEntity.ok(ApiResponse.ok(branchService.listByWorkspace(workspaceId)))
}
